...
func (h *authHandlerImpl) ResetToken(c *fiber.Ctx) error {
	var req resetPasswordByTokenReq
	...
	userID, err := h.resetTokenUc.VerifySession(req.Token)
	if err != nil {
		err = pkgres.Err(err).BadReq()
		return h.log.Log(c, err)
	}

	if err := h.resetTokenUc.ResetPass(userID, req.Password, req.ConfirmPassword); err != nil {
		err = pkgres.Err(err).BadReq()
		return h.log.Log(c, err)
	}
