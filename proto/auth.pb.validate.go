...
func (m *LoginRequest) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetEmailOrPhone()) < 1 {
...
func (m *RegisterRequest) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetConfirmPassword()); l < 6 || l > 50 {
		err := RegisterRequestValidationError{
...
func (m *VerifyAccountRequest) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetToken()) < 1 {
		err := VerifyAccountRequestValidationError{
			field:  "Token",
			reason: "value length must be at least 1 runes",
		}
		if !all {
...
func (m *ForgotPasswordRequest) validate(all bool) error {
	...
	if _, ok := ForgotPasswordType_name[int32(m.GetMethod())]; !ok {
		err := ForgotPasswordRequestValidationError{
			field:  "Method",
...
func (m *ResetPasswordByCodeRequest) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetCode()) < 1 {
		err := ResetPasswordByCodeRequestValidationError{
			field:  "Code",
			reason: "value length must be at least 1 runes",
		}
		if !all {
...
func (m *ResetPasswordByCodeRequest) _validateHostname(host string) error {
	s := strings.ToLower(strings.TrimSuffix(host, "."))
...
func (m *ResetPasswordByCodeRequest) _validateEmail(addr string) error {
	a, err := mail.ParseAddress(addr)
	if err != nil {
		return err
	}
...
func (m *ResetPasswordByTokenRequest) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetToken()) < 1 {
		err := ResetPasswordByTokenRequestValidationError{
			field:  "Token",
			reason: "value length must be at least 1 runes",
		}
		if !all {
...
func (m *CheckCodeRequest) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetCode()) < 1 {
		err := CheckCodeRequestValidationError{
			field:  "Code",
			reason: "value length must be at least 1 runes",
		}
		if !all {
...
func (m *CheckCodeRequest) _validateHostname(host string) error {
	s := strings.ToLower(strings.TrimSuffix(host, "."))
...
func (m *CheckCodeRequest) _validateEmail(addr string) error {
	a, err := mail.ParseAddress(addr)
	if err != nil {
		return err
	}
